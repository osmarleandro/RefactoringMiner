...
public class UMLAnnotation implements Serializable, LocationInfoProvider {
	...
	public String toString() {
		...
		if(!memberValuePairs.isEmpty()) {
			...
			for(String key : memberValuePairs.keySet()) {
				sb.append(key).append(" = ").append(memberValuePairs.get(key).expression);
	...
	public int hashCode() {
		...
		result = prime * result + ((value == null) ? 0 : value.expression.hashCode());
		return result;
	}
	...
	public boolean equals(Object obj) {
		...
		if (value == null) {
			if (other.value != null)
				return false;
		} else if (!value.expression.equals(other.value.expression))
			return false;
		return true;
	}
	...
	private boolean memberValuePairsEquals(UMLAnnotation other) {
		...
		for (Map.Entry<String, AbstractExpression> entry : memberValuePairs.entrySet()) {
			...
			if (thisValue == null) {
				...
				if (!thisValue.expression.equals(m.get(thisKey).expression))
					return false;
			}
		}
		return true;
	...
	private int memberValuePairsHashCode() {
		int h = 0;
		for (Map.Entry<String, AbstractExpression> entry : memberValuePairs.entrySet())
			h += (entry.getKey() == null ? 0 : entry.getKey().hashCode()) ^ (entry.getValue() == null ? 0 : entry.getValue().expression.hashCode());
		return h;
	}
}
