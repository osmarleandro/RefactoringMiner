	...
	protected void createBodyMappers() throws RefactoringMinerTimedOutException {
		...
		for(UMLOperation operation : originalClass.getOperations()) {
			if(!containsMapperForOperation(operation) && nextClass.getOperations().contains(operation) && !removedOperations.contains(operation)) {
    			...
    			if(index != lastIndex) {
    				double d1 = operation.getReturnParameter().type.normalizedNameDistance(nextClass.getOperations().get(index).getReturnParameter().type);
    				double d2 = operation.getReturnParameter().type.normalizedNameDistance(nextClass.getOperations().get(lastIndex).getReturnParameter().type);
    				if(d2 < d1) {
