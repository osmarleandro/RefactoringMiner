...
public class UMLClassDiff extends UMLClassBaseDiff {
	...
	protected void processOperations() {
		for(UMLOperation operation : originalClass.getLocationInfo().getOperations(this)) {
    		if(!nextClass.getLocationInfo().getOperations(this).contains(operation))
    			this.reportRemovedOperation(operation);
    	}
    	for(UMLOperation operation : nextClass.getLocationInfo().getOperations(this)) {
    		if(!originalClass.getLocationInfo().getOperations(this).contains(operation))
	...
	protected void createBodyMappers() throws RefactoringMinerTimedOutException {
		for(UMLOperation originalOperation : originalClass.getLocationInfo().getOperations(this)) {
			for(UMLOperation nextOperation : nextClass.getLocationInfo().getOperations(this)) {
		...
		for(UMLOperation operation : originalClass.getLocationInfo().getOperations(this)) {
			if(!containsMapperForOperation(operation) && nextClass.getLocationInfo().getOperations(this).contains(operation) && !removedOperations.contains(operation)) {
    			int index = nextClass.getLocationInfo().getOperations(this).indexOf(operation);
    			int lastIndex = nextClass.getLocationInfo().getOperations(this).lastIndexOf(operation);
    			int finalIndex = index;
    			if(index != lastIndex) {
    				double d1 = operation.getReturnParameter().getType().normalizedNameDistance(nextClass.getLocationInfo().getOperations(this).get(index).getReturnParameter().getType());
    				double d2 = operation.getReturnParameter().getType().normalizedNameDistance(nextClass.getLocationInfo().getOperations(this).get(lastIndex).getReturnParameter().getType());
    				if(d2 < d1) {
    					finalIndex = lastIndex;
    				}
    			}
    			UMLOperationBodyMapper operationBodyMapper = new UMLOperationBodyMapper(operation, nextClass.getLocationInfo().getOperations(this).get(finalIndex), this);
    			UMLOperationDiff operationSignatureDiff = new UMLOperationDiff(operation, nextClass.getLocationInfo().getOperations(this).get(finalIndex), operationBodyMapper.getMappings());
    			refactorings.addAll(operationSignatureDiff.getRefactorings());
    			this.addOperationBodyMapper(operationBodyMapper);
    		}
    	}
