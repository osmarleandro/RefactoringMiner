	...
	protected void createBodyMappers() throws RefactoringMinerTimedOutException {
		...
		for(UMLOperation operation : originalClass.getOperations()) {
			if(!containsMapperForOperation(operation) && nextClass.getOperations().contains(operation) && !removedOperations.contains(operation)) {
    			...
    			if(index != lastIndex) {
    				double d1 = operation.getJavadoc().getReturnParameter(this).getType().normalizedNameDistance(nextClass.getOperations().get(index).getJavadoc().getReturnParameter(this).getType());
    				double d2 = operation.getJavadoc().getReturnParameter(this).getType().normalizedNameDistance(nextClass.getOperations().get(lastIndex).getJavadoc().getReturnParameter(this).getType());
    				if(d2 < d1) {
