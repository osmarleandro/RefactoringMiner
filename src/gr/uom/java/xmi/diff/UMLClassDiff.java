	...
	protected void createBodyMappers() throws RefactoringMinerTimedOutException {
		...
		for(UMLOperation operation : originalClass.getOperations()) {
			if(!containsMapperForOperation(operation) && nextClass.getOperations().contains(operation) && !removedOperations.contains(operation)) {
    			...
    			if(index != lastIndex) {
    				double d1 = operation.getReturnParameter_RENAMED().getType().normalizedNameDistance(nextClass.getOperations().get(index).getReturnParameter_RENAMED().getType());
    				double d2 = operation.getReturnParameter_RENAMED().getType().normalizedNameDistance(nextClass.getOperations().get(lastIndex).getReturnParameter_RENAMED().getType());
    				if(d2 < d1) {
