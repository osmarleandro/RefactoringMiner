...
public class RenamePackageRefactoring implements Refactoring {
	...
	public Set<ImmutablePair<String, String>> getInvolvedClassesBeforeRefactoring() {
		...
		for(MoveClassRefactoring ref : moveClassRefactorings) {
			pairs.add(new ImmutablePair<String, String>(ref.originalClass.getLocationInfo().getFilePath(), ref.getOriginalClassName()));
		}
		return pairs;
	}
	...
	public List<CodeRange> leftSide() {
		...
		for(MoveClassRefactoring ref : moveClassRefactorings) {
			ranges.add(ref.originalClass.codeRange()
					.setDescription("original type declaration")
					.setCodeElement(ref.originalClass.getName()));
		}
		return ranges;
	}
