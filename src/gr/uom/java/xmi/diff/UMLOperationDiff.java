...
public class UMLOperationDiff {
	...
	private List<SimpleEntry<UMLParameter, UMLParameter>> updateAddedRemovedParameters(UMLOperation removedOperation, UMLOperation addedOperation) {
		...
		for(UMLParameter parameter1 : removedOperation.getParameters_RENAMED()) {
			if(!parameter1.getKind().equals("return")) {
				boolean found = false;
				for(UMLParameter parameter2 : addedOperation.getParameters_RENAMED()) {
		...
		for(UMLParameter parameter1 : addedOperation.getParameters_RENAMED()) {
			if(!parameter1.getKind().equals("return")) {
				boolean found = false;
				for(UMLParameter parameter2 : removedOperation.getParameters_RENAMED()) {
					if(parameter1.equalsIncludingName(parameter2)) {
