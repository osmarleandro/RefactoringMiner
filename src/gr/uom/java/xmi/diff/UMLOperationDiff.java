...
public class UMLOperationDiff {
	...
	private List<SimpleEntry<UMLParameter, UMLParameter>> updateAddedRemovedParameters(UMLOperation removedOperation, UMLOperation addedOperation) {
		...
		for(UMLParameter parameter1 : removedOperation.parameters) {
			if(!parameter1.getKind().equals("return")) {
				boolean found = false;
				for(UMLParameter parameter2 : addedOperation.parameters) {
		...
		for(UMLParameter parameter1 : addedOperation.parameters) {
			if(!parameter1.getKind().equals("return")) {
				boolean found = false;
				for(UMLParameter parameter2 : removedOperation.parameters) {
					if(parameter1.equalsIncludingName(parameter2)) {
