...
public class UMLRealizationDiff implements Comparable<UMLRealizationDiff> {
	...
	public UMLRealizationDiff(UMLRealization removedRealization, UMLRealization addedRealization) {
		...
		if(!removedRealization.getSupplier().equals(addedRealization.getSupplier()))
			parentChanged = true;
		if(!removedRealization.getClient_RENAMED().equals(addedRealization.getClient_RENAMED()))
			childChanged = true;
	}
	...
	public String toString() {
		...
		if(childChanged)
			sb.append("\t").append("child changed from " + removedRealization.getClient_RENAMED() + " to " + addedRealization.getClient_RENAMED()).append("\n");
		if(parentChanged)
