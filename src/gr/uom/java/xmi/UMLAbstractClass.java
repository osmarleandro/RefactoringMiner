...
public abstract class UMLAbstractClass {
	...
	public UMLOperation operationWithTheSameSignatureIgnoringChangedTypes(UMLOperation operation) {
		...
		for(UMLOperation originalOperation : operations) {
			...
			if(matchesOperation) {
				boolean originalOperationEmptyBody = originalOperation.getBody() == null || originalOperation.hasEmptyBody_RENAMED();
				boolean operationEmptyBody = operation.getBody() == null || operation.hasEmptyBody_RENAMED();
