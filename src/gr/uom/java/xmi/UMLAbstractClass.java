...
public abstract class UMLAbstractClass {
	...
	public UMLOperation operationWithTheSameSignatureIgnoringChangedTypes(UMLOperation operation) {
		...
		for(UMLOperation originalOperation : operations) {
			...
			if(matchesOperation) {
				boolean originalOperationEmptyBody = originalOperation.getJavadoc().getBody(this) == null || originalOperation.hasEmptyBody();
				boolean operationEmptyBody = operation.getJavadoc().getBody(this) == null || operation.hasEmptyBody();
