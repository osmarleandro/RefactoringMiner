...
public class LocationInfo {
	...
	public Replacement makeReplacementForReturnedArgument(AbstractCall abstractCall, String statement) {
		if(abstractCall.argumentIsReturned(statement)) {
			return new Replacement(abstractCall.getArguments().get(0), statement.substring(7, statement.length()-2),
					ReplacementType.ARGUMENT_REPLACED_WITH_RETURN_EXPRESSION);
		}
		else if(abstractCall.argumentIsEqual(statement)) {
			return new Replacement(abstractCall.getArguments().get(0), statement.substring(0, statement.length()-2),
