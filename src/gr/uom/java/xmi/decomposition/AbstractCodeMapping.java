				...
				if(invocation != null) {
					for(Refactoring refactoring : classDiff.getRefactoringsBeforePostProcessing()) {
						if(refactoring instanceof RenameOperationRefactoring) {
							...
							if(invocation.getMethodName().equals(rename.getBodyMapper().getRenamedOperation(this).getName())) {
								String initializerBeforeRename = initializer.getString().replace(rename.getBodyMapper().getRenamedOperation(this).getName(), rename.getOriginalOperation().getName());
