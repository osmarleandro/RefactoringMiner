				...
				if(invocation != null) {
					for(Refactoring refactoring : classDiff.getRefactoringsBeforePostProcessing()) {
						if(refactoring instanceof RenameOperationRefactoring) {
							...
							if(invocation.getMethodName().equals(rename.getRenamedOperation().name)) {
								String initializerBeforeRename = initializer.getString().replace(rename.getRenamedOperation().name, rename.getOriginalOperation().name);
