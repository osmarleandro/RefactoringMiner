	...
	private boolean overlappingExtractVariable(AbstractExpression initializer, String input, List<? extends AbstractCodeFragment> nonMappedLeavesT2, Set<Refactoring> refactorings) {
		...
		for(Refactoring ref : refactorings) {
			if(ref instanceof ExtractVariableRefactoring) {
				...
				if(declaration.getInitializer().getInitializer() != null && input.contains(declaration.getInitializer().getInitializer().toString())) {
					output = output.replace(declaration.getInitializer().getInitializer().toString(), declaration.getVariableName());
				}
			}
		}
		...
		if(!longestCommonSuffix.isEmpty() && !longestCommonPrefix.isEmpty() &&
			...
			for(AbstractCodeFragment statement : nonMappedLeavesT2) {
				...
				if(variable != null) {
					if(variable.getInitializer().getInitializer() != null && variable.getInitializer().getInitializer().toString().equals(s1)) {
						return true;
					}
