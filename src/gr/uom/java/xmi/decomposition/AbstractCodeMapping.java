...
public abstract class AbstractCodeMapping {
	...
	private boolean containsIdenticalOrCompositeReplacement() {
		for(Replacement r : replacements) {
			if(r.type.equals(ReplacementType.ARRAY_INITIALIZER_REPLACED_WITH_METHOD_INVOCATION_ARGUMENTS) &&
					r.getBefore().equals(r.getAfter())) {
				return true;
			}
			else if(r.type.equals(ReplacementType.COMPOSITE)) {
				return true;
			}
		}
		return false;
