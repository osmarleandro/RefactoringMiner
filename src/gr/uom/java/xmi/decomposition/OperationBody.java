...
public class OperationBody {
	...
	private void processStatement(CompilationUnit cu, String filePath, CompositeStatementObject parent, Statement statement) {
		if(statement instanceof Block) {
			...
			CompositeStatementObject child = new CompositeStatementObject(cu, filePath, block, parent.depth+1, CodeElementType.BLOCK);
			...
			CompositeStatementObject child = new CompositeStatementObject(cu, filePath, ifStatement, parent.depth+1, CodeElementType.IF_STATEMENT);
			parent.addStatement(child);
			...
			CompositeStatementObject child = new CompositeStatementObject(cu, filePath, forStatement, parent.depth+1, CodeElementType.FOR_STATEMENT);
			parent.addStatement(child);
			...
			CompositeStatementObject child = new CompositeStatementObject(cu, filePath, enhancedForStatement, parent.depth+1, CodeElementType.ENHANCED_FOR_STATEMENT);
			parent.addStatement(child);
			...
			CompositeStatementObject child = new CompositeStatementObject(cu, filePath, whileStatement, parent.depth+1, CodeElementType.WHILE_STATEMENT);
			parent.addStatement(child);
			...
			CompositeStatementObject child = new CompositeStatementObject(cu, filePath, doStatement, parent.depth+1, CodeElementType.DO_STATEMENT);
			parent.addStatement(child);
			...
			StatementObject child = new StatementObject(cu, filePath, expressionStatement, parent.depth+1, CodeElementType.EXPRESSION_STATEMENT);
			parent.addStatement(child);
		}
		else if(statement instanceof SwitchStatement) {
			SwitchStatement switchStatement = (SwitchStatement)statement;
			CompositeStatementObject child = new CompositeStatementObject(cu, filePath, switchStatement, parent.depth+1, CodeElementType.SWITCH_STATEMENT);
			parent.addStatement(child);
			...
			StatementObject child = new StatementObject(cu, filePath, switchCase, parent.depth+1, CodeElementType.SWITCH_CASE);
			parent.addStatement(child);
		}
		else if(statement instanceof AssertStatement) {
			AssertStatement assertStatement = (AssertStatement)statement;
			StatementObject child = new StatementObject(cu, filePath, assertStatement, parent.depth+1, CodeElementType.ASSERT_STATEMENT);
			parent.addStatement(child);
		}
		else if(statement instanceof LabeledStatement) {
			LabeledStatement labeledStatement = (LabeledStatement)statement;
			SimpleName label = labeledStatement.getLabel();
			CompositeStatementObject child = new CompositeStatementObject(cu, filePath, labeledStatement, parent.depth+1, CodeElementType.LABELED_STATEMENT.setName(label.getIdentifier()));
			parent.addStatement(child);
			processStatement(cu, filePath, child, labeledStatement.getBody());
		}
		else if(statement instanceof ReturnStatement) {
			ReturnStatement returnStatement = (ReturnStatement)statement;
			StatementObject child = new StatementObject(cu, filePath, returnStatement, parent.depth+1, CodeElementType.RETURN_STATEMENT);
			parent.addStatement(child);	
		}
		else if(statement instanceof SynchronizedStatement) {
			SynchronizedStatement synchronizedStatement = (SynchronizedStatement)statement;
			CompositeStatementObject child = new CompositeStatementObject(cu, filePath, synchronizedStatement, parent.depth+1, CodeElementType.SYNCHRONIZED_STATEMENT);
			parent.addStatement(child);
			...
			StatementObject child = new StatementObject(cu, filePath, throwStatement, parent.depth+1, CodeElementType.THROW_STATEMENT);
			parent.addStatement(child);
		}
		else if(statement instanceof TryStatement) {
			TryStatement tryStatement = (TryStatement)statement;
			TryStatementObject child = new TryStatementObject(cu, filePath, tryStatement, parent.depth+1);
			parent.addStatement(child);
			...
			for(CatchClause catchClause : catchClauses) {
				...
				CompositeStatementObject catchClauseStatementObject = new CompositeStatementObject(cu, filePath, catchClauseBody, parent.depth+1, CodeElementType.CATCH_CLAUSE);
			...
			if(finallyBlock != null) {
				CompositeStatementObject finallyClauseStatementObject = new CompositeStatementObject(cu, filePath, finallyBlock, parent.depth+1, CodeElementType.FINALLY_BLOCK);
			...
			StatementObject child = new StatementObject(cu, filePath, variableDeclarationStatement, parent.depth+1, CodeElementType.VARIABLE_DECLARATION_STATEMENT);
			parent.addStatement(child);
		}
		else if(statement instanceof ConstructorInvocation) {
			ConstructorInvocation constructorInvocation = (ConstructorInvocation)statement;
			StatementObject child = new StatementObject(cu, filePath, constructorInvocation, parent.depth+1, CodeElementType.CONSTRUCTOR_INVOCATION);
			parent.addStatement(child);
		}
		else if(statement instanceof SuperConstructorInvocation) {
			SuperConstructorInvocation superConstructorInvocation = (SuperConstructorInvocation)statement;
			StatementObject child = new StatementObject(cu, filePath, superConstructorInvocation, parent.depth+1, CodeElementType.SUPER_CONSTRUCTOR_INVOCATION);
			parent.addStatement(child);
		}
		else if(statement instanceof BreakStatement) {
			BreakStatement breakStatement = (BreakStatement)statement;
			StatementObject child = new StatementObject(cu, filePath, breakStatement, parent.depth+1, CodeElementType.BREAK_STATEMENT);
			parent.addStatement(child);
		}
		else if(statement instanceof ContinueStatement) {
			ContinueStatement continueStatement = (ContinueStatement)statement;
			StatementObject child = new StatementObject(cu, filePath, continueStatement, parent.depth+1, CodeElementType.CONTINUE_STATEMENT);
			parent.addStatement(child);
		}
		else if(statement instanceof EmptyStatement) {
			EmptyStatement emptyStatement = (EmptyStatement)statement;
			StatementObject child = new StatementObject(cu, filePath, emptyStatement, parent.depth+1, CodeElementType.EMPTY_STATEMENT);
			parent.addStatement(child);
		}
	}
