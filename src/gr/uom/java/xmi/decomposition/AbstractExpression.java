...
public class AbstractExpression extends AbstractCodeFragment {
	...
	public Replacement makeReplacementWithTernaryOnTheLeft(TernaryOperatorExpression ternaryOperatorExpression, String statement) {
		if(ternaryOperatorExpression.getElseExpression().getString().equals(statement)) {
			return new Replacement(ternaryOperatorExpression.getExpression(), statement, ReplacementType.EXPRESSION_REPLACED_WITH_TERNARY_ELSE);
		}
		if(ternaryOperatorExpression.getThenExpression().getString().equals(statement)) {
			return new Replacement(ternaryOperatorExpression.getExpression(), statement, ReplacementType.EXPRESSION_REPLACED_WITH_TERNARY_THEN);
		}
		return null;
	}
}
