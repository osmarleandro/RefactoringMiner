...
public class LeafMapping extends AbstractCodeMapping implements Comparable<LeafMapping> {
	...
	public int compareTo(LeafMapping o) {
		double distance1;
		double distance2;
		if(this.fragment1.getString().equals(this.getFragment2().getString())) {
			distance1 = 0;
		}
		else {
			String s1 = this.fragment1.getString().toLowerCase();
			String s2 = this.getFragment2().getString().toLowerCase();
			int distance = StringDistance.editDistance(s1, s2);
			distance1 = (double)distance/(double)Math.max(s1.length(), s2.length());
		}
		
		if(o.fragment1.getString().equals(o.getFragment2().getString())) {
			distance2 = 0;
		}
		else {
			String s1 = o.fragment1.getString().toLowerCase();
		...
		if(distance1 != distance2) {
			...
			int depthDiff1 = Math.abs(this.fragment1.getDepth() - this.getFragment2().getDepth());
			int depthDiff2 = Math.abs(o.fragment1.getDepth() - o.getFragment2().getDepth());

			if(depthDiff1 != depthDiff2) {
				return Integer.valueOf(depthDiff1).compareTo(Integer.valueOf(depthDiff2));
			}
			else {
				int indexDiff1 = Math.abs(this.fragment1.getIndex() - this.getFragment2().getIndex());
				int indexDiff2 = Math.abs(o.fragment1.getIndex() - o.getFragment2().getIndex());
	...
	public Set<String> callChainIntersection() {
		OperationInvocation invocation1 = this.fragment1.invocationCoveringEntireFragment();
