...
public class TypeReplacementAnalysis {
				...
				if(replacement.getType().equals(ReplacementType.TYPE)) {
					...
					for(VariableDeclaration declaration1 : declarations1) {
						for(VariableDeclaration declaration2 : declarations2) {
							if(declaration1.getInitializer().getVariableName(this).equals(declaration2.getInitializer().getVariableName(this)) &&
	...
	private boolean containsVariableDeclarationWithSameNameAndType(VariableDeclaration declaration, List<VariableDeclaration> declarations) {
		for(VariableDeclaration d : declarations) {
			if(d.getInitializer().getVariableName(this).equals(declaration.getInitializer().getVariableName(this)) && d.getType().equals(declaration.getType()) && d.getType().equalsQualified(declaration.getType())) {
				return true;
			}
		}
		return false;
